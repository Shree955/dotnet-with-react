...
public class BaseActivityValidator<T, TDto> : AbstractValidator<T> where TDto : BaseActivityDto
{
  ...
  {
      RuleFor(x => selector(x).Title).NotEmpty().WithMessage("Title is required").MaximumLength(100).WithMessage("Title must not esceed 100 characters");
      ...
      RuleFor(x => selector(x).City).NotEmpty().WithMessage("Cityis required");
